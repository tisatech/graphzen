...
describe("# setApprovalAsPrivate", () => {
  ...
  it("should throw when `approvalType` is not PAGE", async () => {
    await expect(item.setApprovalAsPrivate()).to.eventually.rejected;
  });

  it("should not throw", async () => {
    await item.setApprovalPage("Sample Instruction");
    await item.setApprovalAsPrivate();
  });

  it("should modify the correct values", () => {
    expect(item.approval?.privacy).to.be.equal("PRIVATE");
  });
