...
describe("# setApprovalTaskOnly", () => {
  ...
  it("should throw when `approvalType` is not PAGE", async () => {
    await expect(item.setApprovalTaskOnly()).to.eventually.rejected;
  });

  it("should not throw", async () => {
    await item.setApprovalPage("Sample Instruction");
    await item.setApprovalTaskOnly();
  });

  it("should modify the correct values", () => {
    expect(item.approval?.taskOnly).to.be.equal(true);
  });
